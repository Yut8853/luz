...
export default class AJAX extends BaseComponent {
  constructor({ name, loadInnerComponents, parent, element, options }) {
    super({
      name,
      loadInnerComponents,
      parent,
  ...
  _attachEvents() {
    document.addEventListener(
      ...
      this._handlers.onTransitionStart
    );
    document.addEventListener(
      "arts/barba/transition/end",
      this._handlers.onTransitionEnd
    );
    document.addEventListener(
      "arts/barba/transition/init/after",
      this._handlers.onTransitionInit
    );
  }
  ...
  _prevent({ el }) {
    let url = el.getAttribute("href"),
      customRules =
        !!this.options.preventRules && this.options.preventRules.length
          ? AJAXHelpers.sanitizeSelector(this.options.preventRules)
          : null,
      exludeRules = [
        ".has-click-and-hold",
...
class AJAXAnimations {
  static animateCurtains(
    ...
    return new Promise((resolve) => {
      const headerRef = app.componentsManager.getComponentByName("Header"),
        shouldInverseDirection = AJAXAnimations._shouldInverseDirection(
          data,
          headerRef
        );
  ...
  static animateContainersWithOpenedOverlayMenu(
    data,
    {
      duration = 2.4,
      ease = "expo.inOut",
    ...
    return new Promise((resolve) => {
      ...
      AJAXHelpers.resetScrollPosition(0.2).then(() => {
        ...
        const headerTL = headerRef.instance.overlay.timeline,
        ...
        tl.set(data.current.container, {
          ...
          .set(data.next.container, {
            opacity: 1,
            zIndex: 100,
            x: shouldInverseDirection
              ? `-${motionOffsetX}`
              : `${motionOffsetX}`,
            y: shouldInverseDirection
              ? `-${motionOffsetY}`
              : `${motionOffsetY}`,
          })
  ...
  static animateContainers(
    ...
    return new Promise((resolve) => {
      const backgroundColor = gsap.getProperty(
          data.next.container,
          "backgroundColor"
        ),
        tl = gsap.timeline({
      ...
      [...data.current.container.querySelectorAll(".pin-spacer > *")].forEach(
        (el) => {
  ...
  static hideContent(data, { duration = 0.2 } = {}) {
    return new Promise((resolve) => {
  ...
  static animateMedia({
    ...
    return new Promise((resolve) => {
      const tl = gsap.timeline({
        ...
        currentMedia = cloneElement.querySelector(
          ".js-ajax-transition-element__media"
        ),
        currentMask = cloneElement.querySelector(
          ".js-ajax-transition-element__mask"
        ),
        currentOverlay = cloneElement.querySelector(
          ".js-ajax-transition-element__overlay"
        ),
        nextMedia = nextElement.querySelector(
          ".js-ajax-transition-element__media"
        ),
        nextOverlay = nextElement.querySelector(
          ".js-ajax-transition-element__overlay"
        ),
    ...
    ScrollTrigger.getAll()
      .filter(
        (instance) =>
          instance.trigger &&
          data.current.container &&
          data.current.container.contains(instance.trigger)
      )
      .forEach((instance) => {
  ...
  static _shouldInverseDirection(data, headerRef) {
    return (
      data.trigger &&
      data.trigger === "back" &&
      (!headerRef || (headerRef && !headerRef.instance.opened))
    );
  }
}
...
class AJAXHelpers {
  ...
  static removeClonedElements() {
    return new Promise((resolve) => {
      const clones = [
        ...document.querySelectorAll(".js-ajax-transition-clone"),
      ];
...
class AJAXLifecycle {
  ...
  static initNewPage(data) {
    return new Promise((resolve, reject) => {
      ...
      // fallback hard refresh if transition failed
      console.warn(e);
  ...
  static buildContainersDOM(data) {
    return new Promise((resolve, reject) => {
      ...
      const parseCurrentContainerTask = scheduler.postTask(() => {
        data.current.DOM = parser.parseFromString(
          data.current.html,
          "text/html"
        );
      });
      ...
      Promise.all([parseCurrentContainerTask, parseNextContainerTask]).finally(
        () => {
          if (data.next.DOM && data.next.container) {
            data.current.content = data.current.container.querySelector(
              "#page-wrapper__content"
            );
            data.next.content = data.next.container.querySelector(
              "#page-wrapper__content"
            );
            ...
            app.injectPreloadTags({ container: data.next.content }).then(
              () => resolve(true),
              () => resolve(true)
            );
          } else {
...
class AJAXSeamless {
  static getCurrentMediaElements(data) {
    const result = {
      ...
      postID = postIDElement
        ? postIDElement.getAttribute("data-post-id")
        : null;

    let currentElement = data.trigger.querySelector(
    ...
    if (!currentElement) {
      ...
      if (outerWrapper) {
        currentElement = outerWrapper.querySelector(
          ".js-ajax-transition-element"
        );
      }
    }
    ...
    if (!currentElement && postID) {
      ...
      if (outerContainer) {
        const siblings = [
          ...outerContainer.querySelectorAll(`[data-post-id="${postID}"]`),
        ];
        ...
        if (siblings.length) {
          siblings.forEach((el) => {
            ...
            if (el.classList.contains("js-ajax-transition-element")) {
              ...
              const siblingImage = el.querySelector(
                ".js-ajax-transition-element"
              );
    ...
    if (currentElement) {
      ...
      const videoMediaElement = currentElement.querySelector(
        "video.js-ajax-transition-element__media"
      );

      if (videoMediaElement) {
        result.currentElementMedia = videoMediaElement;
      } else {
        const imageMediaElement = currentElement.querySelector(
          "img.js-ajax-transition-element__media"
        );
  ...
  static getNextMediaElements(data) {
    ...
    const nextElement = data.next.container.querySelector(
      ".masthead .js-ajax-transition-element"
    );

    if (nextElement) {
      result.nextElement = nextElement;

      const videoMediaElement = nextElement.querySelector(
        "video.js-ajax-transition-element__media"
      );
      ...
      if (videoMediaElement) {
        ...
        const imageMediaElement = nextElement.querySelector(
          "img.js-ajax-transition-element__media"
        );
  ...
  static translateWebGLPlane(
    data,
    { duration, ease, offsetTop, canvas, nextElement, nextElementMedia } = {}
  ) {
    return new Promise((resolve) => {
      if (canvas instanceof HTMLElement) {
        const nextOverlayEl = nextElement.querySelector(
          ".js-ajax-transition-element__overlay"
        );
        ...
        const event = new CustomEvent("translatePlane", {
          detail: {
            ...
            ease,
            offsetTop:
              typeof offsetTop === "function" ? offsetTop() : offsetTop,
            trigger: data.trigger,
            nextElement,
            nextElementMedia,
            callback: () => {
              ...
              AJAXHelpers.resetScrollPosition().then(() =>
                AJAXHelpers.resolveOnNextTick(resolve)
              );
            },
          },
  ...
  static cloneMedia(
    {
      ...
      position = "fixed",
    } = { target: document.body, position: "fixed" }
  ) {
    return new Promise((resolve) => {
      ...
      const { top, left, width, height } =
        ...
        mediaElement = currentElement.querySelector(
          ".js-ajax-transition-element__media"
        ),
        ...
        nextOverlayEl = nextElement.querySelector(
          ".js-ajax-transition-element__overlay"
        );
      ...
      const hiddenTransitionElements = [
        ...cloneElement.querySelectorAll(".js-ajax-transition-hidden-element"),
      ];
      ...
      if (videoEl) {
        const cloneVideoEl = cloneElement.querySelector("video"),
        ...
        tl.to(cloneElement, {
          ...
          opacity: 1,
          onComplete: () => {
            Promise.all([cloneVideoEl.play(), nextVideoEl.play()])
              .then(() =>
                AJAXHelpers.resolveOnNextTick(resolve, {
                  cloneElement,
                  nextElement,
...
const AJAXTransitionAutoScrollNext = {
  ...
  custom: (data) => {
    if (data && data.trigger instanceof HTMLElement) {
      ...
      return (
        el &&
        el.getAttribute("data-ajax-transition") ===
          AJAXTransitionAutoScrollNext.name
      );
    } else {
      return false;
    }
  ...
  afterEnter: (data) => {
    return new Promise((resolve) => {
      let duration = AJAXHelpers.getTransitionDuration(),
        ease = AJAXHelpers.getTransitionEase(),
        { nextElement, nextElementMedia } =
          AJAXSeamless.getNextMediaElements(data);
      ...
      if (nextElement && nextElementMedia) {
        ...
        if (canvas) {
          AJAXSeamless.setCanvas(canvas)
            ...
            .then(() =>
              AJAXSeamless.loadWebGLPlane(data, {
                canvas,
                nextElement,
                nextElementMedia,
              })
          ...
          const currentElement = data.trigger
            .closest("[data-post-id]")
            .querySelector(".js-ajax-transition-element");

          AJAXSeamless.cloneMedia({ currentElement, nextElement }).then(
            ({ cloneElement, nextElement }) => {
...
const AJAXTransitionFlyingImage = {
  ...
  custom: (data) => {
    if (data && data.trigger instanceof HTMLElement) {
      ...
      return (
        el &&
        el.getAttribute("data-ajax-transition") ===
          AJAXTransitionFlyingImage.name
      );
    } else {
      return false;
    }
  ...
  before: (data) => {
    return new Promise((resolve) => {
      AJAXLifecycle.startTransition(data).then(() =>
        AJAXHelpers.resolveOnNextTick(resolve)
      );
    });
  },
  ...
  enter: (data) => {
    return new Promise((resolve) => {
      AJAXLifecycle.initNewPageFlyingImageBefore(data).then(() =>
        AJAXHelpers.resolveOnNextTick(resolve)
      );
    });
  },
  ...
  afterEnter: (data) => {
    return new Promise((resolve) => {
      ...
      if (nextElement && nextElementMedia) {
        ...
        // WebGL -> Image transition
        if (canvas) {
          const container = canvas.closest(".has-curtains") || canvas,
            translateY = gsap.getProperty(canvas, "y", "px"),
            translateYParsed =
              typeof translateY === "string" ? parseFloat(translateY) : 0,
...
const AJAXTransitionGeneral = {
  ...
  enter: (data) => {
    return new Promise((resolve) => {
      let duration = AJAXHelpers.getTransitionDuration(),
        ease = AJAXHelpers.getTransitionEase();
      AJAXAnimations.animateCurtains(data, { duration, ease }).then(() => {
        AJAXHelpers.resolveOnNextTick(resolve);
      });
    });
  },
...
class AJAXUpdater {
  ...
  static evalInlineScripts(data) {
    return new Promise((resolve) => {
      if (!!app.options.ajax.evalInlineContainerScripts) {
        AJAXUpdater._evalInlineScripts(data.next.container).finally(() =>
          resolve(true)
        );
      } else {
        resolve(true);
  ...
  static _evalInlineScripts(container) {
    return new Promise((resolve) => {
      ...
      const readyPromises = [],
        excludeTypes = ["application/ld+json", "application/json"],
  ...
  static autoPlayPausedVideos(data) {
    return new Promise((resolve) => {
      const videos = [
          ...data.next.container.querySelectorAll("video[muted][autoplay]"),
        ],
        promises = [];
  ...
  static disposeComponents(data) {
    return new Promise((resolve) => {
      ...
      app.componentsManager.instances.disposable =
        app.componentsManager.instances.disposable.filter(
    ...
    return new Promise((resolve) => {
      ...
      nodesToUpdate.forEach((selector) => {
        ...
        if (selector === "#page-header .menu-overlay li" && !nextItems.length) {
          nextItems = [
            ...data.next.DOM.querySelectorAll("#page-header .menu-classic li"),
          ];
        }
  ...
  static _updateBody(data) {
    return new Promise((resolve, reject) => {
      if (
        data.next.DOM.body &&
        data.next.DOM.body.classList.contains("page-no-ajax")
      ) {
        reject(
  ...
  static _updateHeadTags(data) {
    return new Promise((resolve, reject) => {
      ...
      tagstoUpdate.forEach((selector) => {
        ...
        if (currentElement && nextElement) {
          // both tags exist, update (replace) them with the new ones
          currentElement.replaceWith(nextElement);
        } else if (currentElement && !nextElement) {
          // tag doesn't exist on the next page, remove it
          currentElement.remove();
        } else if (!currentElement && nextElement) {
          // tag doesn't exist on the current page but it exists on the next page
  ...
  static _updateHeadStyles(data) {
    let promises = [],
      stylesToUpdate = ['link[rel="stylesheet"][id]', "style[id]"],
      currentStyles = [
        ...document.head.querySelectorAll(stylesToUpdate.join(", ")),
      ],
      nextStyles = [
        ...data.next.DOM.querySelectorAll(stylesToUpdate.join(", ")),
      ];

    // Load & update <head> styles
    nextStyles.forEach((el) => {
      Promise.all(promises).then(() => {
        const currentElement = document.head.querySelector(`#${el.id}`),
          currentHref = currentElement
            ? currentElement.getAttribute("href")
            : null,
          nextHref = el.getAttribute("href"),
          refElement = currentElement
            ? [...document.head.children].filter((el) =>
                el.isEqualNode(currentElement.previousElementSibling)
              )[0]
            : currentElement;
        ...
        if (isInlineStyle) {
          ...
          if (currentElement) {
            ...
            // create new inline script element
            promises.push(
          ...
          // handle style with [href] attribute
  ...
  static _updateScripts(data) {
    return new Promise((resolve) => {
      const promises = [],
        customNodes =
          AJAXHelpers.sanitizeSelector(app.options.ajax.updateScriptNodes) ||
          [],
        nextScripts = [
          ...data.next.DOM.querySelectorAll(
            'script[id], script[src*="maps.googleapis.com/maps/api"]'
          ),
        ];

      nextScripts.forEach((el) => {
        const currentElement = el.id
          ? document.body.querySelector(`#${el.id}`)
          : undefined;
        ...
        if (isInlineScript) {
          ...
          if (currentElement) {
            ...
            if (
              currentElement.textContent !== el.textContent &&
              !el.id.includes("asli")
            ) {
              currentElement.textContent = el.textContent.trim();
    ...
    return new Promise((resolve) => {
      if (
        ...
        targetElements.forEach((element, index) => {
          const targetAttributes = [...element.attributes].filter(
              (attr) => attr.name !== "style"
            ),
            sourceAttributes = [...sourceElements[index].attributes].filter(
              (attr) => attr.name !== "style"
            );
            ...
            sourceAttributes.forEach((attr) => {
              if (attr.nodeName in sourceElements[index].attributes) {
                element.setAttribute(
                  attr.nodeName,
                  sourceElements[index].attributes[attr.nodeName].nodeValue
                );
              } else {
                element.removeAttribute(attr.nodeName);
              }
  ...
  static _updateCloudFlareEmailProtection() {
    return new Promise((resolve) => {
      ...
      function decryptLinks(doc) {
        for (
          var links = doc.querySelectorAll("a"), c = 0;
          c < links.length;
          c++
        )
          try {
