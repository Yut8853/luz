...
class AssetsManager {
  ...
  injectPreload({
    ...
    if (
      src &&
      !document.head.querySelector(`link[rel="${rel}"][href="${src}"]`) &&
...
class BaseAnimation {
  ...
  _registerAnimations() {
    ...
    for (const [name, options] of Object.entries(this.animations)) {
      const elements = this._getScopedAnimationElements(
        `[data-arts-os-animation-name="${name}"]`
      );
      ...
      if (elements.length) {
        ...
        if (
          "setAnimationName" in options &&
          typeof gsap.effects[options.setAnimationName] === "function"
        ) {
          Object.assign(vars, {
        ...
        if (
          "animationName" in options &&
          typeof gsap.effects[options.animationName] === "function"
        ) {
          Object.assign(vars, {
        ...
        this.elements.outlines.forEach((el, index) => {
          timeline.to(
            el,
            {
              ...
              delay: index * 0.1,
              onStart: () => {
                const parentOverflowLine = this.elements.outlines[
                  index
                ].closest(".js-arts-split-text__wrapper-line");
  ...
  _createSTReveal() {
    ...
    if (animation) {
      ...
      masterTimeline.timeScale(
        app.utilities.getTimeScaleByKey("onScrollReveal")
      );

      this.stReveal = ScrollTrigger.create({
        start: this.containerAnimation
          ? `top bottom`
          : `top bottom-=${window.innerHeight * triggerHook}px`,
          ...
          start: ({ trigger }) => {
            const offset =
              trigger.getBoundingClientRect().top < window.innerHeight
                ? 0
                : window.innerHeight * triggerHook;
  ...
  _hasAnimationScene() {
    const animationAttribute = this.element.getAttribute(
  ...
  _getScopedAnimationElements(selector = "[data-arts-split-text-preset]") {
    const containers = [
      ...this.element.querySelectorAll(this._getScopedSelector(selector)),
    ];
...
class BaseComponent extends BaseAnimation {
    ...
    document.fonts.ready
      ...
      .then(() => {
        if (AJAX && AJAX.running) {
          ...
          document.addEventListener(
            "arts/barba/transition/end/before",
  ...
  mount() {
    return new Promise((resolve) => {
      if (this.parent && this.parent.horizontalScroll) {
        ...
        this.containerAnimation = this.horizontalScroll.getContainerAnimation(
          this.element
        );
      }
  ...
  _initLazyMedia() {
    const lazyMedia = [
      ...this.element.querySelectorAll(
        ".lazy:not(:scope [data-arts-component-name] .lazy)"
      ),
    ];
    ...
    if (lazyMedia.length) {
      lazyMedia.forEach((element) => {
        const parent = element.parentElement,
          ...
          isFullheight =
            element.classList.contains("full-height") ||
            element.classList.contains("hs-full-height");
        ...
        if (
          ...
          width &&
          height &&
          !element.closest(".custom-aspect-ratio") &&
          !element.closest(".auto-width-height")
        ) {
  ...
  _initSplitText() {
    ...
    this.splitObserver = new ResizeObserver(
      app.utilities.debounce(this._onSplitTextResize.bind(this), 50)
    );
    ...
    splitTarget.forEach((el, index) => {
      ...
      if (
        presetAttribute &&
        presetAttribute in app.animations.splitTextPresets
      ) {
    ...
    for (const entry of entries) {
      ...
      this.splitTextInstance.forEach((instance) => {
        if (instance.containerElement === entry.target) {
          ...
          if (
            entry.target.classList.contains("split-text-animation-revealed")
          ) {
          ...
          if (
            ...
            [...entry.target.querySelectorAll("u ellipse")].forEach((el) => {
              const parentOverflowLine = el.closest(
  ...
  _updateOptions({
    ...
    if (attributeSelector) {
      ...
      const inlineOptions = app.utilities.parseOptionsStringObject(
        container.getAttribute(attributeSelector)
      );
  ...
  _updateElements({ container, elements }) {
...
class ComponentsManager {
  ...
  loadComponent({
    ...
    const componentName = name || el.getAttribute(nameAttribute),
      attrOptions = options || el.getAttribute(optionsAttribute);

    return new Promise((resolve, reject) => {
      const ComponentClass = componentsMap[componentName];
  ...
  load({ properties = [] }) {
    ...
    return new Promise((resolve) => {
      ...
      Promise.all(depsPromises)
        ...
        .then(() =>
          typeof properties.file === "string" ? import(properties.file) : {}
        )
        .then(resolve);
    });
  }
  ...
  getComponentByName(name) {
    return this.instances.persistent.filter(
  ...
  _setEmptyValue(el, controlWrapper) {
    ...
    if (controlWrapper) {
      controlWrapper.classList.remove(
        this.inputClassFocused,
        this.inputClassNotEmpty
      );
    }
  }
...
class HoverEffect {
  constructor() {
    ...
    this.attachEvents(
      document,
      this._handlers.hoverIn,
      this._handlers.hoverOut
    );
  }
...
class Utilities {
  constructor() {
    this._handlers = {
      resize: this.debounce(
        ...
        this.getDebounceTime(300)
      ),
      orientationchange: this.debounce(
        ScrollTrigger.refresh.bind(ScrollTrigger, false),
  ...
  _attachEvents() {
    ...
    window.addEventListener(
      "orientationchange",
      this._handlers.orientationchange
    );
  }
  ...
  attachResponsiveResize({
    ...
    if (typeof callback === "function") {
      ...
      if (!!autoDetachOnTransitionStart) {
        document.addEventListener("arts/barba/transition/start", clear, {
          once: true,
        });
      }
  ...
  scrollTo({
    ...
    const scrollRef = app.componentsManager.getComponentByName("Scroll"),
      isSmoothScroll =
        this.isSmoothScrollingEnabled() && scrollRef && scrollRef.instance;
  ...
  isEnabledOption(obj) {
    return (
      obj === true ||
      (typeof obj === "object" &&
        (!("enabled" in obj) || ("enabled" in obj && obj["enabled"] === true)))
    );
  }

  getTimeScaleByKey(key) {
    if (
      key in app.options.animations.speed &&
      typeof app.options.animations.speed[key] === "number"
    ) {
      ...
      return gsap.utils.clamp(
        0.01,
        Infinity,
        app.options.animations.speed[key]
      );
    }
  ...
  getTriggerHookValue(defaultValue = 0.15) {
    if (
      "triggerHook" in app.options.animations &&
      typeof app.options.animations.triggerHook === "number"
    ) {
...
(() => {
      ...
      417: (t) => {
        ...
        function o(t, i, c) {
          ((c = c || {}).arrayMerge = c.arrayMerge || n),
            ...
            (c.cloneUnlessOtherwiseSpecified = s);
  ...
  (() => {
    ...
    class n {
      constructor({
        container: t,
        attributeSelector: i = "data-arts-split-text-options",
        options: s,
      }) {
        (this._data = e),
      ...
      _transformOptions({ container: i, attributeSelector: s, options: a }) {
        if (!i) return {};
        let r = {};
        if ((a && e && (r = t(e, a)), s)) {
          let e;
          (e =
            "DATA" === s
              ? (function (t, e = { separator: "-", pattern: /^/ }) {
                  let i = {};
                  var s;
                  return (
              ...
              : n.parseOptionsStringObject(i.getAttribute(s))),
            e &&
              0 !== Object.keys(e).length &&
              ((e = n.transformPluginOptions(e)), (r = t(r, e)));
        }
        this.data = r;
      }
      static parseOptionsStringObject(t) {
        ...
        if (!t) return e;
        try {
          e = JSON.parse(n.convertStringToJSON(t));
        } catch (e) {
    ...
    class a {
      ...
      static getElementsInContainer(t, e) {
        return "string" == typeof e && t && null !== t
          ? [...t.querySelectorAll(e)]
          : [...e];
      }
    }
    function r(t, e = !1) {
      return (e ? t.toLowerCase() : t).replace(/(?:^|\s|["'([{])+\S/g, (t) =>
        t.toUpperCase()
      );
    }
    class o {
      constructor({ container: t, options: e = {} }) {
        (this._enabled = !1),
          (this._initialized = !1),
          t &&
            e &&
            (this._updateContainerElement(t),
            this._updateOptions(t, e),
      ...
      set containerElement(t) {
        this._containerElement = t;
      }
      _updateContainerElement(t) {
        this.containerElement = a.getElementByStringSelector(t);
      }
      get options() {
      ...
      _updateSplitTarget() {
        const t = this.containerElement.children;
        t.length > 0
          ? ([...t].forEach((t) => {
              t instanceof HTMLElement &&
                (t.matches("ul") || t.matches("ol")) &&
                (this._wrapListElements(t),
                (t.dataset.artsSplitTextElement = "list"));
            }),
            (this.splitTarget = [...t]))
          : (this.splitTarget = [this.containerElement]);
      }
      get splitInstance() {
        return this._splitInstance;
      }
      set splitInstance(t) {
        (this._splitInstance = t),
          (this.containerElement.dataset.artsSplitTextReady =
            null === t ? "false" : "true");
      }
      ...
      _markSplitTextElements() {
        "chars" in this.splitInstance &&
          this.splitInstance.chars.length > 0 &&
          this.splitInstance.chars.forEach((t) => {
            t instanceof HTMLElement &&
              "string" == typeof this.options.charClass &&
              t.classList.add(this.options.charClass);
          }),
          "words" in this.splitInstance &&
            this.splitInstance.words.length > 0 &&
            this.splitInstance.words.forEach((t) => {
              t instanceof HTMLElement &&
                "string" == typeof this.options.wordClass &&
                t.classList.add(this.options.wordClass);
            }),
          "lines" in this.splitInstance &&
            this.splitInstance.lines.length > 0 &&
            this.splitInstance.lines.forEach((t) => {
              t instanceof HTMLElement &&
                "string" == typeof this.options.lineClass &&
                t.classList.add(this.options.lineClass);
            });
      }
      _wrapSplitTextElements() {
        if (this.options.wrap && "string" == typeof this.options.wrap) {
          let t = "";
          "string" == typeof this.options.wrap &&
            (t = this.options.wrap.slice(0, -1).toLowerCase()),
            this.splitInstance[this.options.wrap].forEach((e) => {
              if (e instanceof HTMLElement) {
                const i = document.createElement("div");
                "string" == typeof this.options.wrapClass &&
                  i.classList.add(this.options.wrapClass),
                  i.classList.add(`js-arts-split-text__wrapper-${t}`),
                  this._wrap(e, i);
              }
            });
        }
      ...
      _wrapListElements(t) {
        [...t.querySelectorAll("li")].forEach((t) => {
          if (t instanceof HTMLElement) {
            ...
            (e.dataset.artsSplitTextElement = "wrapperLi"),
              this._wrapInner(t, e);
          }
        });
      }
      _wrap(t, e) {
        t.parentNode && (t.parentNode.insertBefore(e, t), e.appendChild(t));
      }
      _wrapInner(t, e) {
        for (t.appendChild(e); t.firstChild !== e; )
          t.firstChild && e.appendChild(t.firstChild);
      }
      _handleDropCap() {
        if ("string" == typeof this.options.dropCapSelector) {
          const t = this.containerElement.querySelectorAll(
            this.options.dropCapSelector
          );
          t.length > 0 &&
            [...t].forEach((t) => {
              const e = t.innerHTML[0],
                i = t.innerHTML.slice(1, -1);
              t.innerHTML = `<span data-arts-split-text-element="wrapperDropCap">${e}</span>${i}`;
            });
        }
      }
      _set() {
        if (
          "artsSplitTextState" in this.containerElement.dataset &&
          "string" == typeof this.containerElement.dataset.artsSplitTextState
        ) {
          const {
              animationName: t,
              selector: e,
              ...
              decodeURIComponent(
                this.containerElement.dataset.artsSplitTextState
              )
            ),
            a = { selector: e, duration: 0, x: i, y: s, autoAlpha: n };
          gsap.effects[t](this.containerElement, a);
        } else if (
          this.options.set &&
          "type" in this.options.set &&
          "string" == typeof this.options.set.type
        ) {
          const t = `hide${r(this.options.set.type)}`;
          if (t in gsap.effects && "function" == typeof gsap.effects[t]) {
            const e = {
              [this.options.set.direction]:
                "number" == typeof this.options.set.distance
                  ? `${this.options.set.distance}px`
                  : this.options.set.distance,
              duration: 0,
            };
            "number" == typeof this.options.set.opacity &&
              (e.opacity = this.options.set.opacity),
              gsap.effects[t](this.containerElement, e);
          }
        }
      }
    }
    class c {
      constructor({ condition: t, callbackMatch: e, callbackNoMatch: i }) {
        (this._handlers = { change: this._onChange.bind(this) }),
          (this.condition = t),
          (this.callbacks = { match: e, noMatch: i }),
          (this._hasMatchFunction() || this._hasNoMatchFunction()) &&
            this.init();
      }
      init() {
      ...
      _attachEvents() {
        "function" == typeof this.mediaQuery.addEventListener
          ? this.mediaQuery.addEventListener("change", this._handlers.change)
          : this.mediaQuery.addListener(this._handlers.change);
      }
      _detachEvents() {
        "function" == typeof this.mediaQuery.removeEventListener
          ? this.mediaQuery.removeEventListener("change", this._handlers.change)
          : this.mediaQuery.removeListener(this._handlers.change);
      }
      _onChange(t) {
        t.matches
          ? this._hasMatchFunction() && this.callbacks.match()
          : t.matches ||
            (this._hasNoMatchFunction() && this.callbacks.noMatch());
      }
    }
    gsap.registerPlugin(ScrollTrigger), gsap.registerPlugin(SplitText);
    var l = function (t, e) {
      var i = {};
      for (var s in t)
    ...
    i(76),
      ...
      new (class {
          ...
          (this._animations = {}),
            ...
            (this._animations = {
              ...
              chars: {
                ...
                y: 0,
                autoAlpha: 1,
                stagger: { from: "end", axis: "x", amount: 0.3 },
              },
            }),
            this._registerAnimations();
        }
        _registerAnimations() {
          for (const t in this._animations) {
            const e = r(t),
              i = `animate${e}`,
              s = `hide${e}`;
            gsap.registerEffect({
              name: i,
              effect: this._effect,
              defaults: Object.assign(Object.assign({}, this._animations[t]), {
                animationName: i,
                type: "reveal",
              }),
              extendTimeline: !0,
              ...
              gsap.registerEffect({
                ...
                defaults: Object.assign(
                  Object.assign({}, this._animations[t]),
                  {
                    animationName: s,
                    y: "-103%",
          ...
          gsap.registerEffect({
            ...
            gsap.registerEffect({
              ...
              effect: this._effectCharsDirectional,
              defaults: Object.assign(
                Object.assign({}, this._animations.chars),
                {
                  y: "-103%",
                  autoAlpha: this._options.hideWithOpacity,
        ...
        _effect(t, e) {
          const i = gsap.timeline({ defaults: { duration: 0 } });
          if (t && t[0]) {
            const s = () => [...t[0].querySelectorAll(e.selector)],
              { selector: n } = e,
              a = l(e, ["selector"]);
            if ("type" in e) {
              const i = a.onStart,
              ...
              "reveal" === e.type &&
                ((a.onStart = () => {
                  "function" == typeof i && i(),
                    t[0].classList.remove("split-text-animation-hidden");
                }),
                (a.onComplete = () => {
                  "function" == typeof s && s(),
                    t[0].classList.remove("split-text-animation-hidden"),
                    t[0].classList.add("split-text-animation-revealed");
                })),
                "hide" === e.type &&
                  ((a.onStart = () => {
                    "function" == typeof i && i(),
                      t[0].classList.remove("split-text-animation-revealed");
                  }),
                  (a.onComplete = () => {
                    "function" == typeof s && s(),
                      t[0].classList.remove("split-text-animation-revealed"),
                      t[0].classList.add("split-text-animation-hidden");
                  })),
                delete a.type;
            }
            return 0 === a.duration
              ? i.add(() => {
                  (t[0].dataset.artsSplitTextState = encodeURIComponent(
                    JSON.stringify(e)
                  )),
                    delete a.stagger,
                    delete a.duration,
                    delete a.animationName,
                    delete a.selector,
                    gsap.set(s(), a);
                })
              : (delete a.animationName,
        ...
        _effectCharsDirectional(t, e) {
          if (t && t[0]) {
            const i = [...t[0].querySelectorAll(e.selector)];
            if (i.length) {
              const { selector: t } = e,
                s = l(e, ["selector"]);
              if (
                ...
                let t;
                switch (gsap.getProperty(i[0], "text-align")) {
                  case "left":
                  default:
                    t = "start";
                    break;
                ...
                s.stagger = (function (t) {
                  ...
                  return function (o, c, l) {
                    ...
                    if (!e) {
                      for (
                        e = [], g = _ = 1 / 0, y = b = -g, S = [], m = 0;
                        m < E;
                        m++
                      )
                        (f = l[m].getBoundingClientRect()),
                          (d = (f.left + f.right) / 2),
                          (u = (f.top + f.bottom) / 2),
                          d < g && (g = d),
                          d > y && (y = d),
                          ...
                          (S[m] = { x: d, y: u });
                      for (
                        h = isNaN(s) ? g + (y - g) * r : S[s].x || 0,
                          p = isNaN(s) ? _ + (b - _) * r : S[s].y || 0,
                          y = 0,
                          g = 1 / 0,
                          m = 0;
                        m < E;
                      ...
                      (e.max = y - g),
                        (e.min = g),
                        (e.v = E = t.amount || t.each * E || 0),
                        (e.b = E < 0 ? n - E : n);
                    }
                    return (
                      (E = (e[o] - e.min) / e.max),
                      e.b + (i ? i.getRatio(E) : E) * e.v
                    );
                  };
                })({ from: t, amount: s.stagger.amount });
              }
              return 0 === s.duration ? gsap.set(i, s) : gsap.to(i, s);
            }
          }
        }
      })();
    const h = class extends o {
      constructor(t, e = {}) {
        super({ container: t, options: e }),
          this.options.init &&
            (this.options.matchMedia &&
            !window.matchMedia(`${this.options.matchMedia}`).matches
              ? (this.matchMedia = new c({
